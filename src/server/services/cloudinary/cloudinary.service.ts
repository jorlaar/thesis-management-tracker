...
class Cloudinary {
     ...
     * @param value {string} value for public id could the accountno, phoneno or verification id
     * @returns {obj} upload cloudinary asset object
     */
    async uploadFile(file: string, folder: string, resource_type: string, value: string) {
        try {
            if (env.app_env !== 'production') return {
                "url": "http://res.cloudinary.com/melas/image/upload/v1667386673/test/v0vony4bk3ytm9pumrok.pdf",
                "secure_url": "https://res.cloudinary.com/melas/image/upload/v1667386673/test/v0vony4bk3ytm9pumrok.pdf",
            };
